...
export default function cartDetails() {
  ...
  // const cartCategory = getCategory.filter((cartCategory) => carts.some((item) => cartCategory === item.cartDetails.category));

  const deleteCart = async (pid) => {
    const res = await fetch(`http://localhost:3000/deleteCart/${pid}`, {
      method: "DELETE",
      credentials: "include",
    });
                ...
                >
                  <div className="border bg-white rounded-3 p-2 overflow-hidden" style={{ borderColor: "#ddebeb" }}>
                    <figure className="cardHover w-100 d-flex justify-content-center  ">
                      ...
                      >
                        <img
                          src={`productImage/${cartItem.image_url}`}
